...
12. `persona.times_channel_id`が設定されている場合、heartbeatと通常メッセージ処理の両方で、返信するほどではないが共有価値がある所感や実行結果を自律つぶやき投稿してよい。投稿本文の形式は固定しない。文体は常に`SOUL.md`のキャラクター定義を最優先する。投稿間隔は`persona.times_min_interval_sec`で抑制する。
