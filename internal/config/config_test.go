...
func TestLoadResolvesRelativeWorkspaceAndHomeFromConfigDir(t *testing.T) {
	dir := t.TempDir()
	cfgPath := filepath.Join(dir, "config.yaml")
	body := `discord:
  token: "token"
...
`
	...
	wantWorkspace, err := filepath.Abs(filepath.Join(dir, "runtime/workspace"))
	if err != nil {
		t.Fatalf("Abs(workspace) error = %v", err)
	}
	wantHome, err := filepath.Abs(filepath.Join(dir, "runtime/.codex-home"))
	if err != nil {
